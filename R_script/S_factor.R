...
# OUTPUT: P, S
PenFactor <- function(ER, L, R, mue, qQ){
...
jj <- coulomb_wave_FG(reta_i, rrho_i, L, k=0)
...
P <- rrho_i/(F_l^2 + G_l^2)
S <- rrho_i*(F_l*Fp.val + G_l*Gp.val)/(F_l^2 + G_l^2);  
return(list(P=P,S=S))  
}
...
plot(x,Sfactor(x,0.0912,2.93,0.0794),log="x",type="l",ylim=c(0,35))
