...
ensamble <- read.csv("ensamble.csv",header = T)  %>%
  ...
  mutate(dat=replace(dat,dat %in% c("gei99b","gei99d"),"Grei99")) %>%
  mutate(dat=replace(dat,dat %in% c("Kra87m","Kra87b"),"Kra87")) %>%
  mutate(dat=replace(dat,dat %in% c("zhi77b"),"Zhi77"))
...
for (k in 1:6){
scale[k] ~ dunif(1-2*syst[k],1+2*syst[k])
}
...
inits <- function () { list(e1 = runif(1,0.3,1),gin=runif(1,0.5,1.5),gout=runif(1,0.001,0.05),scale=runif(6, 0.5, 1.5)) }
...
Normfit <- jags(data = model.data,
              ...
              n.burnin = 5000,
              n.iter = 10000)
mcmcChain <- as.mcmc(Normfit)[,2:4] #Getting e1,gin,gout
...
# One line for each scale (approximate, just using mean value)
    lines( mydat[,1], mydat[,2], col=adjustcolor("black", alpha=0.02))
