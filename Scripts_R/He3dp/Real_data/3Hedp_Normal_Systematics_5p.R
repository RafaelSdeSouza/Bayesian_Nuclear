...
gin ~  dunif(0,10)
gout ~ dunif(0,10)
rf ~  dnorm(5, pow(1,-2))T(0,)
ri ~  dnorm(5, pow(1,-2))T(0,)
...
gdata02 <- data.frame(x =xx, mean = y02[,"mean"],lwr1=y0[,"25%"],lwr2=y02[,"2.5%"],lwr3=y02[,"0.5%"],upr1=y02[,"75%"],
 ...
 gdata0$case <- "Case 1"
 gdata02$case <- "Case 2"
 ROPE0 <- data.frame(x=gdata0$x, c1=gdata0$mean,c2=gdata02$mean) %>% filter(.,x>=0.1998 & x<=0.201)
 d1 <-  density(ROPE0[,2])
 d2 <-    density(ROPE0[,3])
...
ROPE <- data.frame(x=gdata0$x, delta=(gdata0$mean-gdata02$mean)/(gdata0$mean)) %>% filter(.,x>=0.19 & x<=0.21)
