...
# what happens to likelihood if yscat = 4e-6 ??
  yscat1 ~ dnorm(mu_scat, pow(sigma_scat, -2))T(0,)       # sigma is 2e-6 MeVb
  yscat2 ~ dnorm(mu_scat, pow(sigma_scat, -2))T(0,)
  yscat3 ~ dnorm(mu_scat, pow(sigma_scat, -2))T(0,)
  yscat4 ~ dnorm(mu_scat, pow(sigma_scat, -2))T(0,)

  mu_scat    ~  dnorm(0, pow(1e-5, -2))T(0,)
  sigma_scat ~  dnorm(0, pow(1e-5, -2))T(0,)
