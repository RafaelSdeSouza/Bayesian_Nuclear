plot_Sfactor_DREAM <- function(Normfit){
  
  # Plot
  
  df <- NULL
   for(j in 1:length(xx)){
    mux <-  quantile(sfactor3Hedp_5p(xx[j],ssDat[,1],ssDat[,2],ssDat[,3],ssDat[,4],
    ssDat[,5],ssDat[,6],ssDat[,"ue1"]),probs=c(0.0015,0.025, 0.16, 0.5, 0.84, 0.975,0.9985))
    df  <- rbind(df,mux)  
  }
  y <- df
  ...
  df2 <- NULL
  for(j in 1:length(xx)){
    mux2 <-  quantile(sfactor3Hedp_5p(xx[j],ssDat[,1],ssDat[,2],ssDat[,3],ssDat[,4],
                                     ssDat[,5],ssDat[,6],ssDat[,"ue2"]),probs=c(0.0015,0.025, 0.16, 0.5, 0.84, 0.975,0.9985))
    df2  <- rbind(df2,mux2)  
  }
  y2 <- df2
  ...
  df0 <- NULL
  for(j in 1:length(xx)){
    mux0 <-  quantile(sfactor3Hedp_5p(xx[j],ssDat[,1],ssDat[,2],ssDat[,3],ssDat[,4],
                                      ssDat[,5],ssDat[,6],0),probs=c(0.0015,0.025, 0.16, 0.5, 0.84, 0.975,0.9985))
    df0  <- rbind(df0,mux0)  
  }
  y0 <- df0
  ...
  gdata <- data.frame(x =xx, mean = y[,"50%"],lwr1=y[,"16%"],lwr2=y[,"2.5%"],lwr3=y[,"0.15%"],upr1=y[,"84%"],
                      upr2=y[,"97.5%"],upr3=y[,"99.85%"])
  gobs <- data.frame(obsx,obsy,erry,set,lab)
  gobs$set <- as.factor(gobs$set)
  
  
  gdata2 <- data.frame(x =xx, mean = y2[,"50%"],lwr1=y2[,"16%"],lwr2=y2[,"2.5%"],lwr3=y2[,"0.15%"],upr1=y2[,"84%"],
                       upr2=y2[,"97.5%"],upr3=y2[,"99.85%"])
  
  gdata0 <- data.frame(x =xx, mean = y0[,"50%"],lwr1=y0[,"16%"],lwr2=y0[,"2.5%"],lwr3=y0[,"0.15%"],upr1=y0[,"84%"],
