...
c     ---------------------------------------------------
      ...
      common/cca/mue,kt,par1,par2,par3
      common/cci/z0,z1,jj  
...
c     read MCMC samples
      ...
      do 432 j=1,nsamp          ! j labels samples
         read(8,*,end=2222) par1(j),par2(j),par3(j)      
 432  continue
 2222 continue
c      write(6,*) j-1,nsamp
...
c     write first sample to screen to check if input was ok
      write(6,*) ' first sample: Er, g_i^2, g_f^2'
      write(6,*) par1(1)
      write(6,*) par2(1)
      write(6,*) par3(1)
c!!!!!!!!!!!!!!!
...
c 9999 format(9x,f12.6,2x,f11.7,2x,1pe13.6)      
c     ---------------------------------------------------
...
c     sigma: cross section in b
      call sfactor(x,par1(jj),par2(jj),par3(jj),sigma)
c      write(6,*) S_hdp
...
c     [input for 3He(d,p)]
c     *******************************************************
      subroutine sfactor(ecm,e1,gi,gf,sigma)
      implicit double precision (a-h,o-z)

c     resonance energy: e1; reduced widths: gi, gf; units: MeV
      real*8 ecm, e1, gi, gf, sigma

C! donnees H3(d,n)
C!      data a1i,a2i,z1i,z2i/3,2,1,1/,a1f,a2f,z1f,z2f/4,1,2,0/
C!      data e1,q/0.0912,17.59/
C! donnees He3(d,p)

      data hm/20.9008d0/

c     input: radii and angular momenta      
      data ai,af,li,lf /6.,5.,0,2/

c     input: masses, charges, Q-value      
      ...
      sdp2=gitot*gftot/((e1+del-ecm)**2.d0+((gitot+gftot)/2.d0)**2.d0)
