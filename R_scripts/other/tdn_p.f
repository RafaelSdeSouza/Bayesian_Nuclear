...
c     uses single level Breit-Wigner expression with energy-dependent
c     partial widths and the level shift in the denominator of the
c     Lorentzian, i.e., partial widths are "formal", not "observed", 
c     widths; 
c
c     can be used for any single-resonance (particle,particle) reaction
c
c     input in data block is for t(d,n)4He
c
c     I/O:
c     tdn.in:  cm resonance energy, reduced widths in incoming and 
c                 outgoing channel (all in MeV) 
c     tdn.out: cm energy (MeV), S-factor (MeVb) 
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      integer ret
      ret=0
...
c     -----------------------------------------------------------
      ...
      INTEGER LD,LN,I,ne,ret
      REAL*8 ECM(2000),SF(2000)
      REAL*8 ER,GN,GD,r_i,r_f
      REAL*8 m1_i,m2_i,z2_i,z1_i,m1_f,m2_f,z2_f,z1_f
      real*8 jt,jp,jr,Q,ES,EF,SS
      REAL*8 PEK,etpe_i,prat_i,prat_f,mue_i,mue_f,omega
      REAL*8 p_i,p_f,pr_i,pr_f,eta_i,eta_f,rho_i,rho_f,s1,s2
      REAL*8 eta_d,eta_n,rho_d,rho_n,F,FP,G,GP
      real*8 reta_i,rrho_i,reta_f,rrho_f,redw_d,redw_n
      real*8 sr_i,sr_f,s_i,s_f,tapp

C     -----------------------------------------------------
C     INPUT: for t(d,n)4He
C     -----------------------------------------------------
      data m1_i,m2_i/3.016,2.014/
      data m1_f,m2_f/4.0026,1.0087/
      data z1_i,z2_i/1,1/
      data z1_f,z2_f/0,2/
      data r_i,r_f/6.0,5.0/
      data jt,jp,jr/0.5,1.0,1.5/
      data Q/17.589/
      data LD,LN/0,2/
      data ES,EF,SS/0.001,1.001,0.001/     ! careful with energies and ranges  
c     --------------------------------------------------------
c     input file lists resonance cm energy, and reduced widths
      open(unit=8,file='tdn.in',status='unknown')
      read(8,*) er
      read(8,*) redw_d
      read(8,*) redw_n
      close(8)
...
C     ----------------------------------------------------
      eta_d=.15748927D0*z2_i*z1_i*DSQRT(mue_i)
      rho_d=.218735097D0*r_i*DSQRT(mue_i)      
      reta_i=eta_d/(DSQRT(ER))
      rrho_i=rho_d*(DSQRT(ER))
      CALL COULL(DFLOAT(LD),reta_i,rrho_i,F,FP,G,GP)
      pr_i=rrho_i/(F**2+G**2)
      sr_i=rrho_i*((F*FP+G*GP)/(F**2+G**2))
      gd=2.d0*redw_d*pr_i
      
      eta_n=.15748927D0*z2_f*z1_f*DSQRT(mue_f)
      rho_n=.218735097D0*r_f*DSQRT(mue_f)      
      reta_f=eta_n/(DSQRT(ER+q))
      rrho_f=rho_n*(DSQRT(ER+q))
      CALL COULL(DFLOAT(LN),reta_f,rrho_f,F,FP,G,GP)
      pr_f=rrho_f/(F**2+G**2)
      sr_f=rrho_f*((F*FP+G*GP)/(F**2+G**2))
      gn=2.d0*redw_n*pr_f
...
C     ---------------------------------------------------
      DO 200 I=1,NE
        ...
        ecm(i)=es+(i-1)*ss 
        etpe_i=EXP(0.989534267D0*z1_i*z2_i*DSQRT(mue_i/ECM(I)))

        eta_i=eta_d/(DSQRT(ECM(I)))
        rho_i=rho_d*(DSQRT(ECM(I)))
        CALL COULL(DFLOAT(LD),eta_i,rho_i,F,FP,G,GP)
        p_i=rho_i/(F**2+G**2)
        s_i=rho_i*(F*FP+G*GP)/(F**2+G**2)
        prat_i=p_i/pr_i

        eta_f=eta_n/(DSQRT(ECM(I)+q))
        rho_f=rho_n*(DSQRT(ECM(I)+q))
        CALL COULL(DFLOAT(LN),eta_f,rho_f,F,FP,G,GP)
        p_f=rho_f/(F**2+G**2)
        s_f=rho_f*(F*FP+G*GP)/(F**2+G**2)
        prat_f=p_f/pr_f

        tapp=(s_i-sr_i)*redw_d+(s_f-sr_f)*redw_n
C       ---------------------------------------------------
C       PUTTING EVERYTHING TOGETHER
C       ----------------------------------------------------
        S1=PEK*etpe_i*omega*prat_i*prat_f*GD*GN
        S2=((ER-ECM(I)-tapp)**2)+0.25d0*((GD*prat_i+GN*prat_f)**2)
        SF(I)=S1/S2
 200  CONTINUE
...
C     OUTPUT
C     ---------------------------------------------------
      OPEN (UNIT=7,FILE='tdn.out',status='unknown')
      DO 300 I=1,NE
        WRITE (7,1000) ECM(I),SF(I)
 300  CONTINUE
      close (7)
 1000 FORMAT(1X,F7.3,6X,1PE10.3) 

      end
      
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

C     Taken from Bardin et al., Com. Phys. Comm. 3 (1972) 73
C     ******************************************************
      SUBROUTINE COULL(XL,XETA,XRHO,XXF,XXFP,XXG,XXGP)
      REAL*8 XF(10),XG(10),XFP(10),XGP(10),SIGMA(10),XETA,XRHO
      REAL*8 XL,XXF,XXG,XXFP,XXGP
      DIMENSION IEXP(10)
C      WRITE(6,1103)
C1103  FORMAT( ' XL ')
C      WRITE(6,1104)(XL)
C1104  FORMAT(D12.4)
      L=XL
      CALL DFCOUL(L,XETA,XRHO,XF,XFP,XG,XGP,IEXP,SIGMA)
      XXF=XF(L+1)
      XXG=XG(L+1)
      XXFP=XFP(L+1)
      XXGP=XGP(L+1)
C      WRITE(6,1101)
C1101  FORMAT( ' COULL ')
C      WRITE(6,1102)(XF,XFP,XG,XGP,XETA,XRHO)
C1102  FORMAT(6D12.4)
      RETURN
      END
C
...
C      WRITE(6,1110)
C1110  FORMAT( ' DFCOUL')
C      WRITE(6,1111)(F,FP,G,GP,ETA,RO)
C1111  FORMAT(6D12.4)
      CALL DFCZ0(ETA,RO,F0,FP0,G0,GP0,I,S)
C      WRITE(6,1112)(F0,FP0,G0,GP0,ETA,RO)
C1112  FORMAT(6D12.4)
      F(1)=F0
      FP(1)=FP0
      G(1)=G0
 ...
 1000 FORMAT (21H RO NEGATIF OU NUL **)
   ...
   29 WRITE (6,1001)
 1001 FORMAT (42H VALEUR ABSOLUE DE ETA SUPERIEURE A 500 **)
      RETURN
    3 IF(ETA+8.D0)4,5,5
    5 IF(ETA)6,7,6
    7 F0=DSIN(RO)
