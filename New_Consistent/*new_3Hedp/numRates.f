CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
       PROGRAM numRates
c      numerical calculation of broad-resonance reaction rates;
c      rates are calculated from MCMC samples for Breit-Wigner
c      expression of S-factor; if itest=1, rate samples are
c      written to output for select temperatures for plotting
c
c      numRates.in:  input with info on nuclei and integration
c                    parameters       
c      numRates.dat: input of Breit-Wigner samples from MCMC; 
c                    units: Er in MeV, reduced widths in MeV;
c                    first line is a dummy
...
c
c       - always make sure the same masses and Q-value are used
c         in numRates.in and in the last [R-matrix] subroutine
c       !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
c
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
...
c     ---------------------------------------------------
      ...
      real*8 par1(100000),par2(100000),par3(100000)
      real*8 par4(100000),par5(100000),par6(100000)
      real*8 xxx(100000)
      ...
      real*8 M0,M1,mue,kt,fu

      common/intd/kmax,kount,dxsav,xp,yp
      common/cca/mue,kt,par1,par2,par3,par4,par5,par6
      common/cci/z0,z1,jj  
...
c        order in input: e0, er, gf, gi, rf, ri
         read(8,9999,end=2222) par1(j),par2(j),par3(j),
     &          par4(j),par5(j),par6(j)      
 432  continue
 2222 continue
c      write(6,*) j-1,nsamp
...
c     write first sample to screen to check if input was ok
      write(6,*) 'first sample: E0, Er, g_f^2, g_i^2, r_f, r_i' 
      write(6,*) par1(1)
      write(6,*) par2(1)
      write(6,*) par3(1)
      write(6,*) par4(1)
...
c!!!!!!!!!!!!!!!
 ...
 9999 format(9x,f11.8,2x,f11.8,2x,f10.7,3(2x,f9.6))      
c     ---------------------------------------------------
...
c        for tests, output lognormal parameters from rates;
...
c     ---------------------------------------------------
c     OUTPUT FIRST RATE SAMPLE TO SCREEN
c     ---------------------------------------------------
      write(6,*) 
      write(6,*) ' first rate samples'
      do 31 i=1,nt
         write(6,8888) t(i),nasv(1,i) 
 31   continue
 8888 format(1f6.3,3x,1pe9.3)
c     ---------------------------------------------------
      STOP   
      END
...
c     cm energy: x  
c     ------------------------------------------------------
c     enter function between next two lines
c     ------------------------------------------------------
c     sigma: cross section in b
c     order rjags output: e0, er, gf, gi, rf, ri
      call sfactor(x,par1(jj),par4(jj),par3(jj),
     &   par6(jj),par5(jj),sigma)

      dydx=x*sigma*dexp(-x/kt)
...
c     *******************************************************
c     S-factor code from Pierre Descouvemont
c     [input for 3He(d,p)]
c     this routine sets E0=Er
c     *******************************************************
      subroutine sfactor(ecm,e0,gi,gf,ri,rf,sigma)
      implicit double precision (a-h,o-z)

c     resonance energy: e0; reduced widths: gi, gf; units: MeV
      real*8 ecm, e0, gi, gf, ri, rf, sigma
      
      data hm/20.9008d0/

c     input: radii and angular momenta  
      li=0
      lf=2
      ai=ri
      af=rf     

c     input: Q-value, masses, charges  
      data q/18.353053/
      data a1i,a2i,z1i,z2i/3.014932d0,2.01355d0,2,1/
      data a1f,a2f,z1f,z2f/4.00151d0,1.007277d0,2,1/
      ...
      hmi=hm/rmui
      hmf=hm/rmuf
      eta0i=1.44d0*z1i*z2i/(2.d0*hmi)
      eta0f=1.44d0*z1f*z2f/(2.d0*hmf)
      ...
      sdp2=gitot*gftot/((e0+del-ecm)**2.d0+((gitot+gftot)/2.d0)**2.d0)
      sig=2.d0*pi*sdp2/(3.d0*xki**2.d0)
