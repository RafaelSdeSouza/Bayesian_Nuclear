...
#
# - uses the C++ function sfactor3Hedp(E, E0, Er, gi, gf, ri, rf, ue),
...
SfacHedp <- function(ecm, e0, er, gi, gf, ri, rf, ue){
  # input masses, charges, angular momenta
  m1_i = 3.01493 
  m2_i = 2.01355        # masses (amu) of 3He and d
  m1_f = 4.00151 
  m2_f = 1.007277       # masses (amu) of p and 4He
  z1_i = 2 
...
for (i in 1:length(obsx1)) {
  ...
  yt1[i] <- sfactor3Hedp(obsx1[i], e0, er, gi, gf, ri, rf, uea)
}    
...
for (i in 1:length(obsx2)) {
  ...
  ym2[i] <- y.norm2 * yt2[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt2[i] <- sfactor3Hedp(obsx2[i], e0, er, gi, gf, ri, rf, uea)
}    
...
for (i in 1:length(obsx3)) {
  ...
  ym3[i] <- y.norm3 * yt3[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt3[i] <- sfactor3Hedp(obsx3[i], e0, er, gi, gf, ri, rf, uea)
}    
...
for (i in 1:length(obsx4)) {
  ...
  ym4[i] <- y.norm4 * yt4[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt4[i] <- sfactor3Hedp(obsx4[i], e0, er, gi, gf, ri, rf, uea)
}    
...
for (i in 1:length(obsx5)) {
  ...
  ym5[i] <- y.norm5 * yt5[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt5[i] <- sfactor3Hedp(obsx5[i], e0, er, gi, gf, ri, rf, ueb)
}    
...
for (i in 1:length(obsx6)) {
  ...
  ym6[i] <- y.norm6 * yt6[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt6[i] <- sfactor3Hedp(obsx6[i], e0, er, gi, gf, ri, rf, uea)
}    
...
for (i in 1:length(obsx7)) {
  ...
  ym7[i] <- y.norm7 * yt7[i]
  # true S-factor [calculated from theory and then scaled]: 
  yt7[i] <- sfactor3Hedp(obsx7[i], e0, er, gi, gf, ri, rf, ueb)
}    
...
#
  wl_d <- 34.6250*pow(ri, -2)
  wl_p <- 51.9461*pow(rf, -2)

# R-matrix reduced widths:
  gi ~ dnorm(0.0, pow(wl_d, -2))T(0,)      
  gf ~ dnorm(0.0, pow(wl_p, -2))T(0,)      

# channel radius included in sampling; 
# B_c is not sampled; we chose Er = E0
  e0 ~ dnorm(0.0, pow(10, -2))T(0,)        # positive since we see sigma peak 
  er <- e0                                 # set Er = E0
  ri ~ dunif(2.5, 8.0)
  rf ~ dunif(2.5, 8.0)

# screening potential:
  uea ~ dnorm(0.0, pow(0.01, -2))T(0,)     # certainly less than 10 keV
  ueb ~ dnorm(0.0, pow(0.01, -2))T(0,)     # certainly less than 10 keV
...
# extrinsic scatter
  ...
  yscat6 ~ dnorm(0.0, pow(5, -2))T(0,)
  yscat7 ~ dnorm(0.0, pow(5, -2))T(0,)
...
ourmodel <- jags.model(f, data = list(          ## jags wants all data in a list
                ...
                inits = list(e0 = 0.2, gi = 0.36, gf = 0.035, ri = 3.25, rf = 5.77, uea=1e-4, ueb=2e-4), 
...
for ( i in round(runif(500, min=1, max=nsamp)) ) {
   lines(x1,SfacHedp(x1,samplesmat[i,1],samplesmat[i,2],samplesmat[i,4],
    samplesmat[i,3],samplesmat[i,6], samplesmat[i,5], samplesmat[i,8]),
    col=adjustcolor("black", alpha=0.02), lw=0.1)
}

for ( i in round(runif(500, min=1, max=nsamp)) ) {
   lines(x1,SfacHedp(x1,samplesmat[i,1],samplesmat[i,2],samplesmat[i,4],
    samplesmat[i,3],samplesmat[i,6], samplesmat[i,5], 0.0),
...
dens_a <- density(1e6*samplesmat[,7])
a98_a <- quantile(1e6*samplesmat[,7], prob = 0.975)

dens_b <- density(1e6*samplesmat[,8])
a98_b <- quantile(1e6*samplesmat[,8], prob = 0.975)
...
# plot #1          
plot(density(samplesmat[,16]), main="", xlab="", ylab="", xlim=c(0,1.0),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
     )
...
polygon(density(samplesmat[,16]), col=adjustcolor("blue", alpha=0.5))
 ...
 # plot #2                  
plot(density(samplesmat[,17]), main="", xlab="", ylab="", xlim=c(0,1.2),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
     )
...
# plot #3                  
plot(density(samplesmat[,18]), main="", xlab="", ylab="", xlim=c(0,0.4),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
     )
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, yaxs='i', xaxs='i')
...
# plot #4                  
plot(density(samplesmat[,19]), main="", xlab="", ylab="", xlim=c(0,1.2),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
     )
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, yaxs='i', xaxs='i')
...
# plot #7                  
plot(density(samplesmat[,22]), main="", xlab="", ylab="", xlim=c(0,2.0),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
     )
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, yaxs='i', xaxs='i')
legend("topright", legend="Ali01b", pch=NA, cex=1.5)
polygon(density(samplesmat[,22]), col=adjustcolor("blue", alpha=0.5))
