...
### 3 lines below also work
###hist(data, prob=TRUE, ylim=c(0,0.3), xlim=c(0,30), breaks=1000, col="red", 
###    lty=0)
###curve(dlnorm(x, param[1], param[2]), add=TRUE, col="blue", lwd=2)

# for linear x-axis scale, take out log="x"
# first
histRes <- hist(data1, plot=FALSE, breaks=490)
...
# third
histRes <- hist(data3, plot=FALSE, breaks=250)
xvals <- histRes$breaks
yvals <- histRes$counts
xvals <- c(xvals,xvals[length(xvals)])
yvals <- c(0,yvals,0)
...
# second
histRes <- hist(data2, plot=FALSE, breaks=500)
xvals <- histRes$breaks
yvals <- histRes$counts
xvals <- c(xvals,xvals[length(xvals)])
yvals <- c(0,yvals,0)
