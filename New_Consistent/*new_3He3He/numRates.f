...
c      rates are calculated from MCMC samples for quadratic 
...
c                    parameters       
c      numRates.dat: input of S-factor samples from MCMC; 
c                    units: Ecm in MeV, S(E) in MeVb  
...
c     read MCMC samples
      ...
      do 432 j=1,nsamp          ! j labels samples
         read(8,9999,end=2222) alpha(j),beta(j),gamma(j)      
 432  continue
 2222 continue
c      write(6,*) j-1,nsamp
...
c!!!!!!!!!!!!!!!
 ...
 9999 format(11x,f9.6,3x,f10.7,3x,1pe13.6)      
c     ---------------------------------------------------
...
c     cm energy: x  
      ...
      dydx=dexp(-twopieta)*dexp(-x/kt)*(alpha(jj)+x*beta(jj)+
     &                 gamma(jj)*(x**2))
