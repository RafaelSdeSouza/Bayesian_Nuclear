...
numRates_3Heag_table <- function(mcmcobj, vars, N = 1000){
  mcdat_I <- as.data.frame(unlist(as.matrix(mcmcobj)[,vars])) 
  # In context of 3Heag-jags, we are extracting a.scale
  ...
  gdat <-  sapply(Tgrid,function(Tgrid){numRates_3Heag(a,T9 = Tgrid)})
  # Multiple Integration
  # Different temperature at different columns 
  # Recall that numRates_3Heag does integration and return a list of the different reaction rates
  # for EACH chain
