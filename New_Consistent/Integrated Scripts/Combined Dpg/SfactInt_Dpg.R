...
setwd(paste("C:/Users/Hong Kiat/Desktop/UROPS Materials/UROPS 1 - Thermonuclear",
            "Reaction Rates/Working Folder/Integrated Dpg/Combined Dpg",sep = " "))
source("numRates_dpg.R")
source("numRates_dpg_Table.R")
...
## Set this to the working directory where you can find ensamble_DPG.csv. This 
...
Nrate  <- numRates_dpg_table(samplesmat,vars = "a.scale", N = 5000)
...
# S-FACTOR DATA 
######################################################################
pdf("MCMC_Dpg_c.pdf",width=10,height=5,onefile=F)
...
box()
# plot legend
legend(0.12, 3e-7, legend=c( "Bys08", "Cas02", "Ma97", "Sch97" ), 
       pch=c(1, 0, 6, 5), col=c("blue", "red", "green4", "black"))

text(4e-3, 2.0e-6, labels=expression(paste("D(p,",gamma,")",NULL^"3","He")), 
     cex=2.0)
...
plot(1, type="n", lwd=2, col="black",  ylim=c(0, 6), xlim=c(1.8, 2.5),
...
polygon(density(fitvec*1e7), col=adjustcolor("blue", alpha=0.5))

title(xlab=expression(paste(S(0), "  (MeV b)")), line=4.0, cex.lab=2.0)
title(ylab="Probability density", line=4.5, cex.lab=2.0)

mtext(expression(paste("x10"^{-7})), side=1, line=2.0, adj=1.0, cex=1.5)
...
plot(1, type="n", lwd=2, col="black",  
     main="", xlab="", ylab="", axes=FALSE, xlim=c(0.87, 1.1), ylim=c(1,15),
     cex=10.5, cex.main=10.0, cex.lab=2.0, cex.axis=1.3, log="", yaxs='i', xaxs='i')
...
######################################################################
pdf("MCMC_Dpg_f.pdf",width=10, height=6, onefile=F)
...
######################################################################
pdf("MCMC_Dpg_g.pdf",width=10, height=6, onefile=F)
## mar is the margin of individual panels, which sets margin sizes in
##    order bottom, left, top, right
...
# plot #1          
plot(density(samplesmat[,6]*1e7), main="", xlab="", ylab="", xlim=c(0, 5),
...
legend("topright", legend="Bys08", pch=NA, cex=1.5)
polygon(density(samplesmat[,6]*1e7), col=adjustcolor("blue", alpha=0.5))

mtext(expression(paste("x10"^{-7})), side=1, line=2.7, adj=1.05, cex=1.2)

# plot #2                  
plot(density(samplesmat[,7]*1e7), main="", xlab="", ylab="", xlim=c(0, 0.4),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
#title(ylab="Probability", line=2.5, cex.lab=2.3)
legend("topright", legend="Cas02", pch=NA, cex=1.5)
polygon(density(samplesmat[,7]*1e7), col=adjustcolor("blue", alpha=0.5))

mtext(expression(paste("x10"^{-7})), side=1, line=2.7, adj=1.05, cex=1.2)

# plot #3                  
plot(density(samplesmat[,8]*1e7), main="", xlab="", ylab="", xlim=c(0, 10),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
legend("topright", legend="Ma97", pch=NA, cex=1.5)
polygon(density(samplesmat[,8]*1e7), col=adjustcolor("blue", alpha=0.5))

mtext(expression(paste("x10"^{-7})), side=1, line=2.7, adj=1.05, cex=1.2)

# plot #4                  
plot(density(samplesmat[,9]*1e7), main="", xlab="", ylab="", xlim=c(0, 1.5),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
legend("topright", legend="Sch97", pch=NA, cex=1.5)
polygon(density(samplesmat[,9]*1e7), col=adjustcolor("blue", alpha=0.5))
...
#################################################
# Plotting the Autocorrelation Plots (Optional)
##################################################
## Import MCMC samples into a ggs object that can be used by all ggs_ graphical functions
# %>% pipes to tibble
pdf("MCMC_Dpg_h.pdf",width=10, height=6, onefile=F)
...
for (i in 1:6){
  data[,i] <- numRates_dpg(apost,tpts[i])
...
#######################################
# Previously Used Codes (Edited) for Plot b
######################################
par(mfcol=c(1,1), mar=c(4.0,7.0,1.0,6.0), oma=c(0.5,1.0,0.5,1.0), tck=0.02, 
    las=1)
...
# plot legend
legend(0.12, 3e-7, legend=c( "Bys08", "Cas02", "Ma97", "Sch97" ), pch=c(1, 0, 6, 5))
...
for(i in 1:nsamp) fitmat[,i] <- helpmat$y * samplesmat[i,1]
...
# add data BYS08 - diamonds
points( obsx2, obsy2, col="gray40", pch=0, cex=1.2 )
add.error.bars( obsx2, obsy2, 0.0, errobsy2, 0.0, col="gray40" )

# add data SCH97 - squares
points( obsx3, obsy3, col="gray40", pch=6, cex=1.2 )
add.error.bars( obsx3, obsy3, 0.0, errobsy3, 0.0, col="gray40" )

# add data CAS02 - triangles
points( obsx4, obsy4, col="gray40", pch=5, cex=1.2 )
