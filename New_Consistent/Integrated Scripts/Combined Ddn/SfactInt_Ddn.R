...
setwd(paste("C:/Users/Hong Kiat/Desktop/UROPS Materials/UROPS 1 - Thermonuclear",
            "Reaction Rates/Working Folder/Integrated Ddn/Combined Ddn",sep = " "))
source("numRates_ddn.R")
source("numRates_ddn_Table.R")
...
## Set this to the working directory where you can find ensamble_DDN.csv. This 
# is also the directory where the pdf that are outputted will be found in
setwd(paste("C:/Users/Hong Kiat/Desktop/UROPS Materials/UROPS 1 - Thermonuclear",
            "Reaction Rates/Working Folder/Integrated Ddn/Combined Ddn",sep = " "))
## Change the directory accordingly

ensamble <- read.csv("ensamble_Ddn.csv",header = T,fileEncoding="UTF-8-BOM") 
...
theory <- read.table("Arai_ddn_2011.dat", header=FALSE)
...
Nrate  <- numRates_ddn_table(samplesmat,vars = "a.scale", N = 5000)
...
# S-FACTOR FIT + DATA 
######################################################################
pdf("MCMC_Ddn_b.pdf",width=10,height=5,onefile=F)
...
# S-FACTOR DATA 
######################################################################
pdf("MCMC_Ddn_c.pdf",width=10,height=5,onefile=F)
...
plot(1, type="n", lwd=2, col="black",  ylim=c(0, 10), xlim=c(4.7, 5.7),
...
######################################################################
pdf("MCMC_Ddn_f.pdf",width=10, height=6, onefile=F)
...
######################################################################
pdf("MCMC_Ddn_g.pdf",width=10, height=6, onefile=F)
## mar is the margin of individual panels, which sets margin sizes in
##    order bottom, left, top, right
## mar is the margin of individual panels, which sets margin sizes in
##    order bottom, left, top, right
...
# plot #1          
plot(density(samplesmat[,7]*1e2), main="", xlab="", ylab="", xlim=c(0, 0.8),
...
# plot #2                  
plot(density(samplesmat[,8]*1e2), main="", xlab="", ylab="", xlim=c(0, 3),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
...
# plot #4                  
plot(density(samplesmat[,10]*1e2), main="", xlab="", ylab="", xlim=c(0, 2.5),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
...
# plot #5                  
plot(density(samplesmat[,11]*1e2), main="", xlab="", ylab="", xlim=c(0, 6),
     cex=10.5, cex.main=10.0, cex.lab=2.3, cex.axis=1.5, yaxs='i', xaxs='i'
)
title(xlab=expression(paste(sigma [S], " (MeVb)")), line=4.0, cex.lab=2.3, 
      yaxs='i', xaxs='i')
...
# %>% pipes to tibble
pdf("MCMC_Ddn_h.pdf",width=10, height=6, onefile=F)
...
for (i in 1:6){
  data[,i] <- numRates_ddn(apost,tpts[i])
...
require(xtable)
#write.csv(Nrate,"rates_ddn.csv",row.names = F)
