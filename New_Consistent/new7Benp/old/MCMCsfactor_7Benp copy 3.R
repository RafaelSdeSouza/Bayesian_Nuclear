...
#  ym1[i] <- y.norm1 * yt1[i]
  # true S-factor [calculated from theory and then scaled]: 
   yt1[i] <- Sc_a*(Ga_a*Gb_a)/( ((obsx1[i]-Er_a)^2)+((Ga_a+Gb_a)/2)^2 )     
           + Sc_b*(Ga_b*Gb_b)/( ((obsx1[i]-Er_b)^2)+((Ga_b+Gb_b)/2)^2 )
}
...
### polynomial parameters
  Er_a ~ dnorm(0.0,pow(100, -2))
  Ga_a ~ dnorm(0.0,pow(100, -2))
  Gb_a ~ dnorm(0.0,pow(100, -2))
  Sc_a ~ dnorm(0.0,pow(100, -2))

  Er_b ~ dnorm(0.0,pow(100, -2))
  Ga_b ~ dnorm(0.0,pow(100, -2))
  Gb_b ~ dnorm(0.0,pow(100, -2))
  Sc_b ~ dnorm(0.0,pow(100, -2))
...
plot( 1, type="n", lwd=2 , col="black" , xlim=xLim, ylim=yLim, 
       axes=FALSE, main="", xlab = "", ylab = "", log="x" )
...
#100*(Ga*Gb)/( ((obsx1[i]-Er)^2)+((Ga+Gb)/2)^2 )

  lines( xComb, 
         samplesmat[i,"Sc_a"]*(samplesmat[i,"Ga_a"]*samplesmat[i,"Gb_a"])/
              ( ((xComb-samplesmat[i,"Er_a"])^2)+((samplesmat[i,"Ga_a"]+
              samplesmat[i,"Gb_a"])/2)^2 )
       + samplesmat[i,"Sc_b"]*(samplesmat[i,"Ga_b"]*samplesmat[i,"Gb_b"])/
              ( ((xComb-samplesmat[i,"Er_b"])^2)+((samplesmat[i,"Ga_b"]+
              samplesmat[i,"Gb_b"])/2)^2 )     
