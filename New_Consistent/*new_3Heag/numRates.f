...
c      rates are calculated from MCMC samples for 3He(a,g) S-factor; 
c      if itest=1, rate samples are written to output for select 
c      temperatures for plotting; the S-factor is given by the
c      theoretical calculation of Neff 2011, multiplied by the 
...
c      do 9988 kk=1,nsamp
c         write(6,*) norm(kk)
c 9988 continue
c      stop

 2222 continue
c      if(kk-1.ne.nsamp)then
c         write(6,*) 'you are trying to read too many samples'
c         stop
c      endif

      close (8)
      
c     read table of Neff theory S-factors     

      open(unit=12,file='TheoryNeff.dat',status='unknown')

      j=1
 364  read(12,*,end=363) etheo(j),stheo(j)
c      stheo(j)=stheo(j)*1.d-6
      j=j+1
      goto 364
 363  continue
      ntheo=j-1
